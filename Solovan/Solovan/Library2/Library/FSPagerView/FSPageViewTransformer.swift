...
open class FSPagerViewTransformer: NSObject {
    ...
    open func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .overlap:
            ...
            let scale = 1.0 - position*0.05 > 0.7 ? 1.0 - position*0.05 : 0.7
            ...
            attributes.center.y = attributes.center.y + attributes.center.y * (1 - scale)
            
            switch position {
            case -CGFloat.greatestFiniteMagnitude ..< 0: // [-Infinity,-1)
                attributes.center.x = attributes.center.x - 24
            case 0:
                break
            case 1:
                attributes.center.x = attributes.center.x - attributes.size.width * (1 - scale)/2
            case 2:
                if isPad() {
                    attributes.center.x = attributes.center.x - attributes.size.width * (1 - scale)
                } else {
                    attributes.center.x = attributes.center.x - attributes.size.width * (1 - scale)/2
                }
            case 3 ... CGFloat.greatestFiniteMagnitude: // [1,+Infinity)
                attributes.center.x = attributes.center.x - attributes.size.width * (1 - scale)*3/2
            default:
                break
            }
        case .linear:
    ...
    open func proposedInteritemSpacing() -> CGFloat {
        ...
        case .overlap:
            ...
            return -14 //pagerView.itemSize.width * -self.minimumScale * 0.1  
        case .linear:
            guard scrollDirection == .horizontal else {
                return 0
            }
